...
public class Matrix1NornRescalerTest extends TestCase {
	...
	public void testSimpleScalingSymm() throws Exception {
		...
		double[][] A = new double[][] {
				...
				{0., 0., 3.e-9}};
		DoubleMatrix2D AMatrix = F2.make(A);
		MatrixRescaler rescaler = new Matrix1NormRescaler(1.e-3);
	 ...
	 */
	public void testSimpleScalingSymm2() throws Exception {
		log.debug("testSimpleScalingSymm2");
		DoubleFactory2D F2 = DoubleFactory2D.dense;
		Algebra ALG = Algebra.DEFAULT;
		double[][] A = new double[][] {
				{-5.179606612022133,-0.02504297960603635,24.852749934356265,-0.0022539455614544743},
				{-0.025042979606036346,-5.164290426157521,-25.64347279666032,0.003046228687793972},
				{24.852749934356265,-25.64347279666032,-242.263120568969,-0.01274431884602755},
				{-0.0022539455614544743,0.003046228687793972,-0.01274431884602755,5.549439055094576E-4}};
		DoubleMatrix2D AMatrix = F2.make(A);
		MatrixRescaler rescaler = new Matrix1NormRescaler(1.e-3);
		DoubleMatrix1D U = rescaler.getMatrixScalingFactorsSymm(AMatrix);
		DoubleMatrix2D AScaled = ColtUtils.diagonalMatrixMult(U, AMatrix, U);
	 ...
	 */
	public void testSimpleScalingSymm3() throws Exception {
		log.debug("testSimpleScalingSymm3");
		DoubleFactory2D F2 = DoubleFactory2D.dense;
		Algebra ALG = Algebra.DEFAULT;
		double[][] A = new double[][] {
				{15.228317792554328, 0.0},
		...
		log.debug("norm0: " + norm0);
		log.debug("norm1: " + norm1);
		assertTrue(rescaler.checkScaling(ColtUtils.fillSubdiagonalSymmetricMatrix(AMatrix), U, U));
		assertFalse(cn1 > cn0);
		assertFalse(norm1 > norm0);
	}
